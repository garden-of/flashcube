  ...
  container_definitions = jsonencode([
    {
      ...
      logConfiguration  = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = "/ecs/flashcube-api"
          awslogs-region        = var.region
          awslogs-stream-prefix = "api-log-stream"
        }
      }
    },
    {
      name              = "flashcube-migrate"
      image             = var.docker_image_url_django
      essential         = false
      cpu               = 128
      memory            = 128
      ...
      logConfiguration  = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = "/ecs/flashcube-migrate"
          awslogs-region        = var.region
    ...
    },
    {
      ...
      logConfiguration  = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = "/ecs/flashcube-nginx"
          awslogs-region        = var.region
...
resource "aws_ecs_service" "stage" {
  ...
  load_balancer {
    ...
    container_name   = "flashcube-nginx"
    container_port   = 80
  }
}